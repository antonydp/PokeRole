...
export const AchievementsBlock: React.FC<AchievementsBlockProps> = ({ achievements, onAchievementChange, onAddAchievement, onRemoveAchievement }) => (
    <div className="bg-slate-800/50 rounded-lg p-1">
        <div className="flex justify-between items-center mb-1">
            <h3 className="text-sm font-bold text-poke-yellow font-primary">Achievements & Notes</h3>
            <button
                onClick={onAddAchievement}
                className="flex items-center justify-center px-1.5 py-0.5 bg-green-600 text-white font-primary text-xs rounded-md border-b-2 border-green-800 hover:bg-green-500 active:translate-y-px active:border-b-0 transition-all duration-150"
            >
        ...
        <div className="space-y-1">
            {(achievements || []).map((achievement, index) => (
                <div
                    key={index}
                    className={`flex items-center space-x-1 p-1 rounded-md transition-all duration-300 ${
                ...
                >
                        ...
                        >
                            {achievement.completed && (
                                <div className="relative w-3.5 h-3.5">
                                    ...
                                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-gray-800 rounded-full z-10"></div>
                                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-3.5 h-3.5 rounded-full border border-gray-800 z-10"></div>
                                    <div className="absolute top-1/2 left-0 right-0 h-px bg-gray-800 z-10"></div>
                                </div>
                            )}
                        </div>
                    </div>
                    <LabeledInput
                        ...
                        containerClassName={`flex-grow border-0 focus:border-yellow-400 rounded px-1.5 py-0.5 text-xs font-bold ${
                            achievement.completed
