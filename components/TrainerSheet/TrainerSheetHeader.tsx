...
const InfoField: React.FC<{ label: string; value: string; onChange: (value: string) => void; placeholder: string }> = ({ label, value, onChange, placeholder }) => (
    <div className="flex-1 min-w-[100px]">
        ...
        <input
            ...
            className="w-full bg-transparent text-white font-primary text-sm p-0 focus:outline-none border-b border-transparent focus:border-poke-yellow transition-colors"
        />
    </div>
);

const MoneyDisplay: React.FC<{ value: string; onChange: (value: string) => void }> = ({ value, onChange }) => (
    <div className="bg-black/40 p-1 rounded-lg border-2 border-black/60 shadow-inner h-full flex flex-col justify-center">
        <label className="text-green-300/80 text-[10px] font-bold uppercase tracking-widest">Poké Dollars</label>
        <div className="flex items-center gap-1 mt-0.5">
            <span className="text-green-300 font-sans text-lg font-bold">₽</span>
            <input
                type="text"
                value={value ?? ''}
                onChange={e => onChange(e.target.value)}
                placeholder="0"
                className="w-full bg-transparent text-white font-sans text-base font-bold p-0 text-right focus:outline-none"
                aria-label="Money"
            />
        </div>
    </div>
...
const StatCalculationDisplay: React.FC<{ label: string; base: number; attributeValue: number; result: number }> = ({ label, base, attributeValue, result }) => (
    <div className="flex items-center justify-between bg-black/20 px-1 py-px rounded">
        <span className="font-bold text-xs text-cyan-200/80">{label}:</span>
        <div className="font-sans font-bold text-xs text-white text-right">
            <span>{base}</span>
...
const TrainerSheetHeader: React.FC<TrainerSheetHeaderProps> = ({ trainerData, onDataChange, onOpenNatureModal }) => {
    ...
    return (
        <div className="bg-gradient-to-b from-stone-800 to-stone-900 p-0.5 rounded-xl border-2 border-stone-900 shadow-2xl font-primary relative mb-1.5">
            <div
                className="absolute top-1/2 -translate-y-1/2 left-1.5 w-20 h-20 bg-gray-700 rounded-full border-4 border-white shadow-lg flex items-center justify-center z-10 overflow-hidden cursor-pointer"
            ...
            <div className="bg-[#B2483D] p-1.5 rounded-lg border-2 border-[#3A3A3A] grid grid-cols-1 md:grid-cols-3 gap-1.5 relative md:pl-24">
                {/* Column 1: ID Panel */}
                <div className="flex flex-col justify-between h-full pt-8 md:pt-0">
                     <input
                        ...
                        className="w-full bg-transparent p-0 pb-0 text-xl font-primary text-white border-b-2 border-white/30 focus:ring-0 focus:outline-none focus:border-poke-yellow placeholder:text-white/50"
                        aria-label="Trainer Name"
                    />
                    <div className="grid grid-cols-2 gap-x-1.5 mt-0.5">
                       ...
                       <div className="flex-1 min-w-[100px]">
                           ...
                           <div className="relative">
                               <select
                                   ...
                                   className="w-full bg-transparent text-white font-primary text-sm p-0 focus:outline-none border-b border-transparent focus:border-poke-yellow transition-colors appearance-none"
                               >
                ...
                <div className="flex flex-col justify-between h-full space-y-1">
                    <div className="flex items-stretch gap-1">
                        <div className="flex-1">
                             <div className="bg-gradient-to-b from-slate-700 to-slate-800 p-1 rounded-lg border-2 border-slate-600 shadow-inner h-full flex flex-col justify-center">
                                ...
                                <div className="flex items-center gap-1 mt-0.5 relative">
                                    ...
                                    <select
                                        ...
                                        className="w-full bg-transparent text-white font-sans text-sm p-0 focus:outline-none appearance-none cursor-pointer"
                ...
                <div className="bg-black/40 rounded-lg p-1.5 border-2 border-black/50 shadow-inner flex flex-col justify-between h-full">
                    <div>
                        <label className="text-cyan-200/70 text-[10px] font-bold uppercase tracking-widest text-center block">Pokédex</label>
                        <div className="flex items-baseline justify-center gap-1 text-white">
                            <div className="text-center">
                                <input
                                    ...
                                    className="bg-transparent font-sans text-lg font-bold text-center w-full focus:outline-none"
                                    aria-label="Pokémon Caught"
                                />
                                <span className="text-xs text-white/60 block -mt-0.5">Caught</span>
                            </div>
                            ...
                            <div className="text-center">
                                ...
                                />
                                <span className="text-xs text-white/60 block -mt-0.5">Seen</span>
                            </div>
                        </div>
                    </div>
