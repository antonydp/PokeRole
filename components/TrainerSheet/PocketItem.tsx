...
const PocketItem: React.FC<PocketItemProps> = ({ itemInstance, itemDetails, onQuantityChange, onRemove, onMouseEnter, onMouseLeave }) => {
    ...
    return (
        <div
            ...
            className="bg-stone-800/10 p-0.5 rounded-md flex items-center gap-1 font-sans animate-fade-in"
        >
            {imageUrl ? (
                <img src={imageUrl} alt={itemInstance.name} className="w-5 h-5 object-contain bg-white/20 rounded-full p-0.5 flex-shrink-0" />
            ) : (
            ...
            <div className="flex items-center gap-px flex-shrink-0">
                <button
                    ...
                    className="w-4 h-4 bg-stone-300 text-stone-800 rounded flex items-center justify-center hover:bg-poke-red hover:text-white transition-colors"
                ...
                <button
                    onClick={() => onQuantityChange(itemInstance.id, itemInstance.quantity + 1)}
                    className="w-4 h-4 bg-stone-300 text-stone-800 rounded flex items-center justify-center hover:bg-green-600 hover:text-white transition-colors"
            ...
            <button
                ...
                className="p-0.5 text-stone-500 hover:bg-red-600 hover:text-white rounded-md transition-colors"
